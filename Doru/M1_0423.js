...
webform.validators.m1 = function (v, allowOverpass) {
    ...
    function validateCAEM_COL1_CAP1(values) {
        ...
        // Use the Select2 API to retrieve the selected CAEM value from both fields
        values.CAEM = jQuery('#CAEM').select2('val');  // Get CAEM from the main activity
        var cap1_caem_c2_value = jQuery('#CAP1_CAEM_C2').select2('val');  // Get the value from the second field (Col 2)

        console.log('Main CAEM value:', values.CAEM);
        ...
        if (values.CAEM !== cap1_caem_c2_value) {
            // Push error if CAEM does not match
            webform.errors.push({
                'fieldName': 'CAP1_CAEM_C1',
                'msg': Drupal.t(`Cod eroare: A.014 Cod CAEM (${values.CAEM}) trebuie sa fie acelasi ca si in Activitatea principală (${cap1_caem_c2_value})`)
            });
        }
    }
