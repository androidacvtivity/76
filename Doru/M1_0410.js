...
webform.validators.m1 = function (v, allowOverpass) {
    ...
    function validateCAEM_COL1_CAP1(values) {
        ...
        var fields_CAP1_CAEM2 = jQuery('#CAP1 thead tr td:nth-child(1)').find('select').val();

        // Check if the CAEM value exists in the 'values' object
        if (!values.CAEM) {
            // Push error if CAEM is missing
            webform.errors.push({
                'fieldName': 'CAEM', // The field that triggers the error
                'msg': Drupal.t('Cod eroare: A.011 Selectati Cod CAEM:')
            });
        }

        // Check if the CAEM value matches the field 'fields_CAP1_CAEM2'
        if (values.CAEM !== fields_CAP1_CAEM2) {
            // Push error if CAEM does not match
            webform.errors.push({
                'fieldName': 'CAP1_CAEM_C1', // Specific field causing the error
                'msg': Drupal.t(`Cod eroare: A.014 Cod CAEM  trebuie sa fie acelasi ca si in Activitatea principală (${fields_CAP1_CAEM2})`)
            });
        }
    }
