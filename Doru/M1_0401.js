...
//------------------------------------------------
    ...
    columns.forEach(col => {
        ...
        // Automatically assign the value from Capitolul 1 to Capitolul 2 if not already filled
        if (cap1Field && !cap2Field) {
            if (cap2Element) {
                cap2Element.value = cap1Field; // Assign CAEM value from Capitolul 1 to Capitolul 2
                console.log(`Assigned CAP2_CAEM_${col} = ${cap1Field}`); // Debug log
            }
        }

        // Validate if the CAEM values in Capitolul 1 and Capitolul 2 are equal
