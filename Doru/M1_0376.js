(function ($) {
    Drupal.behaviors.m1 = {
        attach: function (context, settings) {
            ...
            jQuery('#mywebform-edit-form').on('mywebform:sync', 'select.Section2-caem', function () {
                fill_section3_caem_fields(jQuery(this));
            });
...
function fill_section3_caem_fields($element) {
    var caem = $element.val();

    jQuery('select.Section3-caem3')
        .myWebformSelect2SetVal(caem)
        .trigger('change');
}
    ...
    if (TRIM == 3) {
        ...
        for (var i = 1; i <= 9; i++) {
            if (!isNaN(parseFloat(values["CAP2_R" + (i) + "0_C1"]))) {
                var R010_90_C1 = parseFloat(values["CAP2_R" + (i) + "0_C1"]);
            }
            let sum_R010_90_C2_12 = 0;
            for (let s = 2; s <= 12; s++) {
                sum_R010_90_C2_12 += parseFloat(values["CAP2_R" + (i) + "0_C" + (s)]);
            }
            sum_R010_90_C2_12 = roundToDecimal(sum_R010_90_C2_12, 1);
        ...
        for (var i = 10; i <= 16; i++) {
            ...
            sum_R100_160_C2_12 = roundToDecimal(sum_R100_160_C2_12, 1);
